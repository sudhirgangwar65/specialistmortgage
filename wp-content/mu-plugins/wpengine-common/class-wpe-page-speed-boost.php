...
class Wpe_Page_Speed_Boost {
	...
	public function register_hooks() {
		...
		if ( ! is_admin() && ! defined( 'WP_CLI' ) && ! is_user_logged_in() ) {
			return;
		}
	...
	public function get_connect_url( $url = '' ) {
		return $this->get_product_page_url() . '/add' . ( defined( 'PWP_NAME' ) ? '?selected=' . PWP_NAME : '' );
	}
	...
	public function get_vendor_widget( $html = '' ) {
		return '<h5 class="card-title">'
			...
			. __( 'NitroPack OneClick is provided as part of your Page Speed Boost add-on and is preconfigured with essential features for immediate use. Activate it effortlessly and enjoy and instant boost in page speed.', 'wpengine' )
			. '</div>'
	...
	/**
	 * Page Speed Boost optimized URLs.
	 *
	 * @param array $args       Command arguments.
	 * @param array $assoc_args Command parameters.
	 */
	...
	/**
	 * Page Speed Boost excluded URLs.
	 *
	 * @param array $args       Command arguments.
	 * @param array $assoc_args Command parameters.
	 */
